...
BOOST_AUTO_TEST_CASE(TestWith)
{
    ...
    .OnTrigger(test_pair)
        ...
        .At(test_pair)      .Return(1)
            .And(test_pair) .Return(0).Expect()
    ...
    .OnTrigger(test_map)
        ...
        .At(test_map)       .Inject()
        .At(test_map)       .Return("/0/1")
            .And(test_map)  .Return("/1/1").Expect()
...
BOOST_AUTO_TEST_CASE(TestUnion)
{
    ...
    .OnTrigger(test)
        .At(test).Inject()
        .At(&Mock::foo).CallCount().Alias("f")
            .Or (&Mock::bar).CallCount().Alias("b").Expect()
...
BOOST_AUTO_TEST_CASE(TestFixedChannels)
{
    ...
    .At(sut) .Return("/1") .Alias("a")
    .Or(sut) .Return("/2") .Alias("b")
        .Expect(L{
            {"a",  1},
            {"b", 42},
        })
