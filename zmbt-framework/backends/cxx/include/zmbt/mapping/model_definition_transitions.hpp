...
template <class Target>
struct ModelDefinition::T_Or : protected virtual ModelDefinition::BaseTransition
{
    ...
    template <class... T>
    Target
    Or(boost::json::string_view key, T&&... fmtargs)
    {
        state().combine_channels("union");
        state().add_channel(key | expr::Format(fmtargs...));
        return transit_to<Target>();
    }
};

template <class Target>
struct ModelDefinition::T_And : protected virtual ModelDefinition::BaseTransition
{
    /// Pack channel outputs into an array similarly to Fork keyword
    /// X with Y with Z -> [X, Y, Z]
    template <class C>
    require_not_str<C, Target>
    And(C&& cal, object_id const& obj)
    {
    ...
    template <class... T>
    Target
    And(boost::json::string_view key, T&&... fmtargs)
    {
        state().combine_channels("with");
        state().add_channel(key | expr::Format(fmtargs...));
        return transit_to<Target>();
