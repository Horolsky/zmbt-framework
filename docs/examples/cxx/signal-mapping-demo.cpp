...
The SignalMapping model provides the following functionality for batch testing:
 ...
 - Output combination clauses like `And` and `Or`.
...
It can be made with clauses like `And` or `Or`, which begin an channel definition similar to `At`.
It is possible to chain multiple channels in a signal pipe with a single condition, but mixing different types in a chain will not work.
...
The `Or` chain merges captured samples in a time series, producing a list of pairs `[id, signal]`, sorted by timestamp.
Id, if not defined explicitly with `Alias` clause, is a channel absolute index (ignoring pipe boundaries).

In combination with `Saturate` or other custom matchers, the `Or` output can be used for testing a strict or partial order on mock calls.
...
The `And` clause will simply pack the outputs of the combined channels into an
array.
