...
MODEL = SignalMapping, OnTrigger, [Repeat], {Signal_Pipe}-, [Tests], [Params], [Tasks], [Description];


' Pipe = InitialChannel, {FollowingChannel}-, [(Inject | Expect | ExpectOne | ExpectRange)];
' InitialChannel = At, {Filters};
' FollowingChannel = (And, {Filters}) | (Or, {Filters}) ;

Signal_Pipe  = Init_Channel, [{And_Channel}- | {Or_Channel}-], (Inject | Expect | Assert);
Init_Channel = At, {Filters};
And_Channel  = And, {Filters};
Or_Channel   = Or, {Filters};

Filters = (([(Return | Args | Exception)], [As]) | ThreadId | Timestamp | CallCount), [Via], [Alias];
...
## Signal pipes and channels

Signal pipe is a specific input or output test condition with corresponding
configuration through through which the test runner interacts with SUT.

The pipe configuration is a set of interface endpoints, signal filters,
signal transformation rules, and identifiers for referencing.
...
that holds the corresponding test condition expression.
...
.At(&Producer::get_x)
    .Or(&Producer::get_y)
    .Or(&Producer::get_z)
.Inject(MyGenerator) // 3-channel input pipe

.At(&Consumer::set_x)
    .Or(&Consumer::set_y).Args(0)
    .Or(&Consumer::set_z).Return().As(type<MyType>)
.Expect(MyMatcher)  // 3-channel output pipe with additional filters
```
...
#### Alias
 ...
 - [Alias](/CxxRef/structzmbt_1_1mapping_1_1ModelDefinition_1_1T__Alias/): specify how the channel is referenced in log and in `Or` output.
...
In :construction: `And` pipes, each channel has a dedicated counter.
In :construction: `Or` pipes, all channels share the same counter.
